...
/// Quiz form widget for advanced create post screen
/// Allows users to create a quiz with question and 3 choices (A, B, C)
class QuizFormWidget extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<QuizFormWidget> createState() => _QuizFormWidgetState();
}

class _QuizFormWidgetState extends ConsumerState<QuizFormWidget> {
  ...
  void initState() {
    ...
    _isExpanded = state.qaQuestion?.isNotEmpty == true;
  }
  ...
  int _getTextLength() {
    final text = widget.postText ?? ref.read(createPostProvider).text;
  ...
  void _clearQuiz() {
    ...
    ref.read(createPostProvider.notifier).clearQuiz();
  }
  ...
  Future<void> _generateQuizWithAI() async {
    ...
    if (text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('กรุณาใส่รายละเอียดก่อน')),
      );
      return;
    }

    ref.read(createPostProvider.notifier).setLoadingQuizAI(true);
    ...
    try {
      ...
      if (isValidResult && mounted) {
        ...
        ref.read(createPostProvider.notifier).setAiQuizPreview(
        ...
        if (mounted) {
          ref.read(createPostProvider.notifier).setLoadingQuizAI(false);
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
  ...
  void _applyAiQuizToForm() {
    ...
    ref.read(createPostProvider.notifier).applyAiQuizToForm();
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    OutlinedButton.icon(
                      ...
                      style: OutlinedButton.styleFrom(
                        ...
                        backgroundColor:
                            _canUseAI ? Color(0xFFF4F2FD) : AppColors.alternate,
                        side: BorderSide(
                            color: _canUseAI
                                ? Color(0xFFB0A8C9)
                                : AppColors.alternate),
                ...
                TextField(
                  ...
                  onChanged: (value) {
                    ref.read(createPostProvider.notifier).setQaQuestion(value);
                  },
                ),
                ...
                _buildChoiceField(
                  ...
                  onChanged: (value) {
                    ref.read(createPostProvider.notifier).setQaChoiceA(value);
                  },
                  onSelectAnswer: () {
                    ref.read(createPostProvider.notifier).setQaAnswer('A');
                  },
                ),
                const SizedBox(height: 8),
                ...
                _buildChoiceField(
                  ...
                  onChanged: (value) {
                    ref.read(createPostProvider.notifier).setQaChoiceB(value);
                  },
                  onSelectAnswer: () {
                    ref.read(createPostProvider.notifier).setQaAnswer('B');
                  },
                ),
                const SizedBox(height: 8),
                ...
                _buildChoiceField(
                  ...
                  onChanged: (value) {
                    ref.read(createPostProvider.notifier).setQaChoiceC(value);
                  },
                  onSelectAnswer: () {
                    ref.read(createPostProvider.notifier).setQaAnswer('C');
                  },
                ),
  ...
  Widget _buildAiQuizPreview(CreatePostState state) {
