...
/// หน้าเช็คลิสต์ V2 - รายการงาน (ใหม่)
/// แสดง Tasks ตาม view mode (งานถัดไป, ทั้งหมด, ติดปัญหา, ที่เราติ๊ก)
class ChecklistScreenV2 extends ConsumerStatefulWidget {
  const ChecklistScreenV2({super.key});

  @override
  ConsumerState<ChecklistScreenV2> createState() => _ChecklistScreenV2State();
}

class _ChecklistScreenV2State extends ConsumerState<ChecklistScreenV2> {
  ...
  void _subscribeToRealtimeUpdates() {
    ...
    TaskRealtimeService.instance.subscribe(
      onTaskUpdated: () {
        // Refresh tasks when other NA updates a task (V2 uses v3_task_logs_simplified)
        // ใช้ container แทน ref เพื่อหลีกเลี่ยง disposed ref error
        if (mounted && _container != null) {
          refreshTasksWithContainerV2(_container!);
        }
      },
    );
  }
                ...
                IconButton(
                  ...
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (_) => const RlsDebugScreen()),
                    );
                  },
                ),
              ],
