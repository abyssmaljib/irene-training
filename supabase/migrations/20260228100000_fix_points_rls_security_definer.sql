-- ============================================================
-- Migration: Fix Points RLS - SECURITY DEFINER for RPC functions
-- ============================================================
-- ปัญหา: get_leaderboard และ get_user_tier เป็น SECURITY INVOKER (default)
-- แต่ query ตาราง Point_Transaction ซึ่งมี RLS policy จำกัดการเข้าถึง
--
-- ผลกระทบ:
-- 1. get_leaderboard: user ธรรมดาเห็นแค่ transaction ของตัวเอง → leaderboard มีแค่ 1 คน
-- 2. get_user_tier: ถ้า user ไม่ผ่าน RLS policy จะเห็น 0 คะแนน
--
-- Fix: เปลี่ยนเป็น SECURITY DEFINER + SET search_path = public
--      เพิ่ม auth check ป้องกัน anonymous access
-- ============================================================

-- ============================================================
-- 1. Fix get_leaderboard → SECURITY DEFINER
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    -- ใช้ tier จาก cache ถ้ามี, fallback ไป fixed threshold
  ...
  LEFT JOIN LATERAL (
    ...
    LIMIT 1
  ) ptier ON utc.user_id IS NULL  -- ใช้ fallback เฉพาะเมื่อไม่มี cache
...
-- ============================================================
-- 2. Fix get_user_tier → SECURITY DEFINER
-- ============================================================
...
BEGIN
  ...
  -- คำนวณ total points (ตอนนี้ bypass RLS เพราะเป็น SECURITY DEFINER)
  ...
  IF v_has_cache THEN
    ...
    SELECT
      ...
      utc.tier_icon,
      utc.tier_color,
      0::INT AS min_points,  -- ไม่ใช้แล้วใน percentile mode
      utc.next_tier_name,
      -- backward compat: ประมาณ next_tier_min_points = rolling + gap
  ...
  ELSE
    -- Fallback: ใช้ fixed threshold เดิม (สำหรับ user ที่ยังไม่ถูกคำนวณ)
    RETURN QUERY
...
-- ============================================================
-- 3. GRANT execute ให้ authenticated users
-- ============================================================
...
-- ============================================================
-- 4. Revoke จาก anon เพื่อป้องกัน anonymous access
-- ============================================================
