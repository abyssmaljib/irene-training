...
alter table "public"."training_content" drop constraint "training_content_season_id_fkey";

alter table "public"."training_content" drop constraint "training_content_topic_id_season_id_key";

alter table "public"."training_questions" drop constraint "training_questions_thinking_type_check";

drop view if exists "public"."training_v_topic_detail";

drop index if exists "public"."idx_content_season";

drop index if exists "public"."training_content_topic_id_season_id_key";

alter table "public"."training_content" drop column "season_id";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.set_active_season_on_training_questions()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
declare
...
begin
  if new.season_id is null then
    ...
    if v_season_id is null then
      raise exception 'No active training season found';
    end if;
...
create or replace view "public"."training_v_topic_detail" as  SELECT t.id AS topic_id,
    ...
    ( SELECT count(*) AS count
          ...
          WHERE ((q.topic_id = t.id) AND (q.is_active = true) AND ((q.season_id IS NULL) OR (q.season_id = s.id)))) AS question_count
   FROM (((public.training_topics t
     LEFT JOIN public.training_seasons s ON ((s.is_active = true)))
     LEFT JOIN public.training_content c ON (((c.topic_id = t.id) AND (c.is_active = true))))
...
CREATE TRIGGER trg_set_active_season_on_training_questions BEFORE INSERT ON public.training_questions FOR EACH ROW EXECUTE FUNCTION public.set_active_season_on_training_questions();


  create policy "all 18lvwur_0"
  on "storage"."objects"
  as permissive
  for select
  to public
using ((bucket_id = 'testupload'::text));



  create policy "all 18lvwur_1"
  on "storage"."objects"
  as permissive
  for update
  to public
...
using ((bucket_id = 'testupload'::text));
  ...
  create policy "all 18lvwur_3"
  on "storage"."objects"
  as permissive
  for insert
  to public
with check ((bucket_id = 'testupload'::text));



  create policy "all 4q2b7k_0"
  on "storage"."objects"
  as permissive
  for select
  to public
using ((bucket_id = 'user_Profile_Pic'::text));



  create policy "all 4q2b7k_1"
  on "storage"."objects"
  as permissive
  for insert
  to public
with check ((bucket_id = 'user_Profile_Pic'::text));



  create policy "all 4q2b7k_2"
  on "storage"."objects"
  as permissive
  for update
  to public
